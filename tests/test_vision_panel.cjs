...
assert(html.indexOf('id="main-logs"')<html.indexOf('id="alarm-content"'));
assert(css.includes('.bottom-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr))'));
assert(html.indexOf('id="alarm-content"')<html.indexOf('id="vision-panel"'));
assert(html.indexOf('id="vision-panel"')<html.indexOf('id="page-manual"'));
for(const id of ['main-logs','alarm-content','vision-panel','vision-result','vision-reason','vision-time'])assert.equal(html.split('id="'+id+'"').length-1,1);
