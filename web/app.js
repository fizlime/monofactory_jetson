...
function renderMimics(){const html=plant.process_order.map(c=>stationHtml(c)).join('');const main=$('#main-mimic');if(main&&!main.dataset.three)renderHtml(main,html);renderHtml($('#manual-mimic'),plant.process_order.map(c=>stationHtml(c,true)).join(''))}
function renderMain(){const l=plant.line;$('#kpi-status').textContent=l.status;$('#kpi-message').textContent=displayText(l.message);$('#kpi-active').textContent=l.active_process||'—';$('#kpi-process-name').textContent=l.active_process?displayText(plant.processes[l.active_process].name):'대기';$('#kpi-count').textContent=l.cycle_count;$('#kpi-lamp').textContent=l.lamp||'OFF';renderHtml($('#main-process-flow'),plant.process_order.map(code=>{const p=plant.processes[code],pct=Math.max(0,Math.min(10,Math.round((Number(p.progress)||0)/10)))*10;return `<button data-inspect="${code}" class="process-card ${esc(p.status)} progress-${pct}" title="${displayEsc(p.message||p.summary)}"><div class="pc-top"><span class="code">${code}</span><span class="state">${esc(p.status)}</span></div><h3>${displayEsc(p.name)}</h3><p>${displayEsc(p.message||p.summary)}</p><small>${p.repeat_total>1?`${p.repeat_current}/${p.repeat_total}`:`${p.progress||0}%`}</small></button>`}).join(''));
  const mainLogs=$('#main-logs');if(mainLogs)mainLogs.innerHTML=plant.logs.slice(0,10).map(x=>`<div class="log-row"><span>${esc(x.time)}</span><b>${displayEsc(x.process||x.type)}</b><div>${displayEsc(x.message)}</div></div>`).join('')||'<div class="empty">이벤트 없음</div>';
  ...
  globalThis.MainDashboard?.update(plant);
  globalThis.Line3D?.update(plant);
}
...
function bind(){
  ...
  document.addEventListener('click',async e=>{
    ...
    if(nav){document.body.dataset.activePage=nav.dataset.page;globalThis.Line3D?.setVisible(nav.dataset.page==='main');$$('.nav').forEach(x=>x.classList.remove('active'));nav.classList.add('active');$$('.page').forEach(x=>x.classList.remove('active'));$(`#page-${nav.dataset.page}`).classList.add('active');if(nav.dataset.page==='setting')renderSettings();if(nav.dataset.page==='log')renderLog();return}
  ...
  document.addEventListener('input',e=>{
    ...
    if(e.target.dataset.config){const current=deepGet(draft,e.target.dataset.config);const value=typeof current==='boolean'?e.target.value==='true':e.target.type==='number'||typeof current==='number'?(e.target.dataset.config.endsWith('.home_forward_mm')&&e.target.value===''?null:Number(e.target.value)):e.target.value;deepSet(draft,e.target.dataset.config,value);dirty=true;updateDistanceSummaries()}
